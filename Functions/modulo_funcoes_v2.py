...
def registrar(arquivo):
	...
	while resp == 'S':
		try:
			filme = [input('\n Digite o nome do filme: ').title().replace(',',' ').strip(' '), int(input(' Digite o ano do filme: '))]
...
def formata_exibicao(lista_ordenada):
	...
	for filme in lista_ordenada:
		print(f' {filme[0]:.<50} ({filme[1][:-1]})')	#elimina o '\n' do final
...
def ordenar(arquivo, ordem_ano):
	...
	lista_filmes = [linha.split(',') for linha in lista]
#ordena por ano	
	if ordem_ano == True:	
...
#pesquisa por ano
			...
			resultado = [filme for filme in lista_ordenada if str(filme[1][:-1]) in pesquisa]
			formata_exibicao(resultado)	
		else:
			print(' Opção inválida!\n')		
