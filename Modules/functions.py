...
def registrar(arquivo):
    ...
    while resp == 'S':
        try:
            filme = [input('\n Digite o nome do filme: ').title().replace(',', ' ').strip(' '),
                     int(input(' Digite o ano do filme: '))]
...
def persistir(lista, arquivo):
    with open(arquivo, 'a') as lista_csv:
        writer = csv.writer(lista_csv, lineterminator='\n')
...
def formata_exibicao_indice(lista_ordenada):
    ...
    for filme in lista_ordenada:
        ...
        print(f' {filme[0]} - {filme[1][0]:.<50} ({filme[1][1]})')
    print(f'\n Total:{len(lista_ordenada)} filmes\n')
...
def ordenar(arquivo, ordem_ano):
    ...
    with open(arquivo, 'r', encoding='utf-8') as lista_csv:
        reader = csv.reader(lista_csv)
#        lista = lista_csv.readlines()
        lista_filmes = [linha for linha in reader]
...
def buscar(arquivo):
 ...
 -> '''))
        ...
        if tipo_busca == 1:
            ...
            resultado = [filme for filme in lista_ordenada if pesquisa.lower() in filme[0].lower()]
            formata_exibicao(resultado)
        # pesquisa por ano
        elif tipo_busca == 2:
            pesquisa = input(' Digite o ano a ser pesquisado: ')
            resultado = [filme for filme in lista_ordenada if str(filme[1]) in pesquisa]
            formata_exibicao(resultado)
        else:
            print(' Opção inválida!\n')
...
def excluir(arquivo):
    ...
    resposta = input(
        f' Deseja excluir o filme {lista_numerada[indice][1][0]} ({lista_numerada[indice][1][1]})? (S/N) ').upper()
    if resposta == 'S':
